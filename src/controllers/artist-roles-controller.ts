...
type SingularArtistRoleRequestHandlerResponse = { artistRole: ArtistRoleViewModel } | ErrorResponseBody;
...
export const updateArtistRole: UpdateArtistRoleRequestHandler = async (req, res) => {
  ...
  try {
    const artistRoleDoc = await ArtistRoleModel.findByIdAndUpdate(id, artistRoleProps, { new: true });
    if (artistRoleDoc === null) {
